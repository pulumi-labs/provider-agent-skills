...
| `awaiting/bridge` | An existing bridge issue matches the same failure mode and maintainer next action.<br>Or: Pulumi and Terraform behavior are established, and a focused bridge artifact shows that the remaining gap is bridge-owned. | The bridge is only a plausible layer.<br>TF parity is unknown and no matching bridge issue independently settles ownership.<br>A new bridge artifact is still needed to establish the first bridge-owned failure. |
